  ...
  return (
    <div className="space-y-8">
      {Object.entries(groupedTemplates).map(([category, templates]) => (
        <div key={category} className="p-6 bg-muted/30 rounded-lg border border-border">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {templates.map((template) => (
              <Card key={template.id} variant="elevated" className="hover:shadow-lg transition-all hover:border-primary/20" data-testid={`form-template-${template.id}`}>
                <CardHeader>
